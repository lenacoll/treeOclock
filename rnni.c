...
int nni_move(Tree *tree, long r, int child_moves_up) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (upper_node->children[i] != r) {
      ...
      tree->node_array[lower_node->children[child_moves_up]].parent = r + 1;
...
int rank_move(Tree *tree, long r) {
  if (tree->node_array[r].parent == r + 1) {
    printf("Error. No rank move possible. The interval [%ld,%ld] is an "
           "edge!\n",
           r, r + 1);
    return EXIT_FAILURE;
  }
...
long rnni_distance(Tree *start_tree, Tree *dest_tree) {
  ...
  for (long i = num_leaves; i < num_nodes; i++) {
    ...
    // mrca
    current_mrca_rank = mrca(current_tree, dest_tree->node_array[i].children[0],
                             dest_tree->node_array[i].children[1]);
    Node *current_mrca;
...
// returns tree after first FindPath move IN RNNI (no length moves!)
Tree *first_findpath_move(Tree *start_tree, Tree *dest_tree) {
  long num_leaves = start_tree->num_leaves;
  Tree *current_tree = new_tree_copy(start_tree);
  int move_done = FALSE;
  long i = num_leaves;
  while (move_done == FALSE && i < 2 * num_leaves - 1) {
    long current_mrca = mrca(current_tree, dest_tree->node_array[i].children[0],
                             dest_tree->node_array[i].children[1]);
    if (current_mrca != i) {
      decrease_mrca(current_tree, dest_tree->node_array[i].children[0],
                    dest_tree->node_array[i].children[1]);
